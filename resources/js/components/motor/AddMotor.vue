<template>
    <div>
        <h1>tambah data</h1>
        <form @submit.prevent="handleSubmit" action="/motor/create" method="post" enctype="multipart/form-data">
...
data(){
    ...
    methods: {
        handleSubmit(){
            ...
            axios.post('/api/motor/create', this.form).then((response)=>{
               if(response.data.status){
                console.log(response)
                this.$noty.success("Data Tersimpan")
                    this.$router.push({
                    name: 'motor',
                        })
               }
